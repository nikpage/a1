...
- Wiring: `analysis.js` imports `scenarioList(hasJobText)` + `scenarioHandling(hasJobText)` so the chosen scenario actually shapes positioning and the blueprint (not just a label); `cv-generator.js` imports `scenarioGenerationRules(scenario_tags)` so the per-scenario mitigations reach the CV. Gating by `hasJobText` keeps job-relative scenarios out of standalone reviews.
