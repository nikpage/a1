...
- **`applyVoice()` is the FALLBACK, not a stage.** It runs only when the finished draft measures flat (`coverShapeFaults`) or walks the career (`coverBreadthFault`) — see Layer 6 checks 24 and 25. When it runs it REWRITES THE WHOLE LETTER under an explicit fact lock (cutting is permitted, altering is not), capped at two calls, discarding a rewrite that is wreckage or that did not improve the measured shape. Span-patching is dead for style: what reads as machine-written is SHAPE, and no span replacement can split a paragraph or land a four-word sentence; span-patching stays correct for the truth passes. It runs **before** the truth-verify pass, never after: a style rewrite must not smuggle a claim past the fact-checker. Cost-logged like every other call.
