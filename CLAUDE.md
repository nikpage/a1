...
- **Only `Formal` is offered.** `OFFERED_TONES` in `prompts/tone.js` is the single source of truth for the tone selector (`pages/me.js`, `components/ToneDocModal.js` both import it). Friendly, Enthusiastic and Cocky are HIDDEN, not deleted: tested on a real record with a saved voice profile, Friendly came back indistinguishable from Formal and Cocky produced none of the swagger its own definition demands, because the candidate's voice profile now owns the writing prompt and a voice built from their real prose beats a line asking for a different mood. Offering a tone that changes nothing is a lie to the user. Their definitions stay so an older document still regenerates in the tone it was written in. Re-offer one only after it demonstrably differs on a real run, and log that run.
...
- **The WRITER owns the voice — it is not a pass applied afterwards.** The profile and the candidate's own writing sit at the TOP of `prompts/cover-letter.js`, above the rules, with the shape stated as targets before a word is written (at least one sentence ≤6 words, wide variation, no paragraph past ~90 words, no stub-chopping, open on a concrete thing done). This replaced a two-model design — one drafting cold, one restyling the draft — which produced fragments, orphan one-line paragraphs and, once, a real achievement moved to the wrong employer. Two owners for one document is the defect; do not reintroduce it.
- **`applyVoice()` is now the FALLBACK, not a stage.** It runs only when the finished draft measures flat (`coverShapeFaults`) or walks the career (`coverBreadthFault`) — see Layer 6 checks 24 and 25. When it runs it REWRITES THE WHOLE LETTER under an explicit fact lock (cutting is permitted, altering is not), capped at two calls, discarding a rewrite that is wreckage or that did not improve the measured shape. Span-patching was tried first and is **dead for style**: what reads as machine-written is SHAPE, and no span replacement can split a paragraph or land a four-word sentence. It stays correct for the truth passes. It runs **before** the truth-verify pass, never after: a style rewrite must not smuggle a claim past the fact-checker. Cost-logged like every other call.
...
- **The cover letter's own slice (checks 17–23)** runs through `validateCoverLetter()` in the same file: banned phrasing, the market word band (the one hard failure), matched pairs, salutation, one objection, stray numbers, and **check 23 — an invented domain**. Check 23 does NOT warn: like check 17 it is the app's own writing failing, so `unsourcedDomainHits()` feeds `repairUnsourcedDomains()` in `utils/openai.js` (one narrow `GEMINI_VERIFY_MODEL` call through `buildPhraseRepairPrompt({ kind: 'domain' })`, corrections applied by literal string match) and the word is gone before delivery. The hit list is a **closed list** of industry labels (`DOMAIN_TERMS`), matched by diacritic-folded 6-character stems so CZ/PL inflection counts as one word, against the **master alone**. The ad is NOT a source: Blogic's own blurb says "specialisté na FinTech", and allowing the ad let the letter claim fintech as the candidate's background on the strength of the employer's self-description. An ad names where the EMPLOYER works — evidence about them, never about the applicant. It exists because a real run wrote "fintech" over an ad that asked for financial advisory: a bare domain noun carries no number and no upgraded verb, so no AI verify category looks at it. The list is closed on purpose — judging every word by whether the sources used it flags ordinary prose, and a warning that fires on a normal Czech verb teaches the candidate to ignore the banner. The repair never substitutes a different industry (there is no source for one either) — it says what the candidate did and drops the label. Grow the list only by adding a term actually seen invented.
- **The letter is COMPOSED, not executed** (`CV_RULES.md`, Layer 3). The analysis hands the letter EVIDENCE — `generation_framework.cover_evidence` = `requirement_evidence[]` (up to five unranked `{requirement, evidence}` the record genuinely answers) + `concerns[]` (`{flag, answer_evidence}`, only those the record can answer) — rendered by `prompts/cover-evidence.js`. It decides nothing: the hook, which requirements are used and in what order, whether a concern is raised at all, and the close are the writer's, chosen at write time when the steering, tone, voice profile and market length are finally all present. It replaced `cover_blueprint`, which named the hook, three ordered pairs, the one objection and the close — the whole letter, decided during analysis before the candidate had typed a word, leaving the writer arranging prose around a skeleton it had not chosen and every downstream user input able only to redress a decision already taken. **Do not reintroduce a field that decides the letter's shape.** The salutation is unaffected: it is a fact read off `job_extraction.hr_contact` by `salutationName()`, never a plan's pick.
- **Steering is in scope when the document is composed** (`CV_RULES.md`, Invariants). The emphasise / play-down boxes compose into `tweak` (`utils/steering.js`) and reach both generators, stated concretely because the evidence pool and `prompts/job-target.js` were still gathered before the candidate typed: demoted content is not evidence (use other real evidence, or leave the requirement unanswered); emphasised content leads and is proved; demotion is never deletion (T2 keeps every role and date); steering never adds a fact. `job-target.js` carries the matching exception. **Check 19 no longer takes `tweak` and is never suspended** — it warns only when the letter answers NONE of the answerable requirements, which is true regardless of steering. Tests: `__tests__/cover-provenance.test.js`, `__tests__/cover-evidence.test.js`.
