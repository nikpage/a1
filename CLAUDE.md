...
- **Source split (teaser reads raw, deep reads master):** the landing **teaser** (`analyzeTeaser`) is fed the **raw extracted CV text + the `cv_layout` sidecar** (column count, scanned/image flag, verbatim date strings via `formatLayoutForPrompt`), never the master. Its job is to judge the real document an ATS/recruiter receives — column scramble, unreadable dates, buried credentials — and the cleaned master erases exactly those faults, so a master-fed teaser falsely passes both gates. The **deep pass** reads the master, where interpretive/structural calls belong on the strong model. The master build records facts only: it never decides overlapping roles are one consultancy — overlaps are surfaced as a `role_overlap` open question in `conflicts` for the user.
