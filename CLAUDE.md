...
- **Build = PURE EXTRACTION (no analysis):** the worker builds the master from the raw CV text the first time it's absent, persists it, cost-logs it. The build is transcription only — every role kept separate with verbatim dates, gaps intact, overlaps surfaced as a `role_overlap` conflict; it NEVER merges two roles into one consultancy or makes any interpretive/structural call. Because it's pure extraction (verifiable, backstopped by the verify pass), it correctly runs on the cheap `GEMINI_MASTER_MODEL` (flash-lite) — the model is right for the job; the old bug was the build *interpreting*, not the model being weak. Falls back to raw text only if the build fails.
- **Source split (DATA from the master, GRAPHICS from the layout):** both the **teaser** (`analyzeTeaser`) and the **deep pass** (`analyzeCvJob`) read the **master** for every DATA judgment — dates, gaps, tenures, role overlaps, seniority. This is safe precisely because the build is faithful: the data faults survive into the master (two short overlapping roles stay two roles + a `role_overlap` conflict), so the teaser reasons over clean structured facts instead of sloppily parsing raw text. The **layout signal** carries only the GRAPHICS the master can't hold — column scramble, scanned/image — and drives ONLY the ATS gate. **It is never stored:** computed at upload (`extractCvWithLayout`, geometry only the file shows), returned by `/api/upload-cv`, ridden in on the `analyse-background` request body, read once, discarded. A re-analysis with no fresh upload has no layout → the ATS gate judges on the record alone (fine).
- **One CV, one profile:** every upload mints/refreshes the identity and rebuilds the master from that single CV (`uploadAndAnalyze` → `/api/upload-cv`). The multi-CV add-to-profile / merge / conflict-resolution flow (`AddCvChoiceModal`, `MergeConflictModal`, `/api/add-cv`, `/api/add-cv-commit`, `/api/profile-status`) was removed — `prompts/master-cv.js` still has a `merge` mode if it's ever reinstated, but no live path calls it. Identity always from `req.user`.
