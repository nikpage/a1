...
- **Build:** the background analysis worker builds the master from the raw CV text the first time it's absent, persists it, cost-logs it, then feeds it into the **deep pass** (`analyzeCvJob`). Falls back to raw text only if the build fails. The **teaser does NOT read the master** — see the source-split note below.
- **Source split (teaser reads raw, deep reads master):** the landing **teaser** (`analyzeTeaser`) is fed the **raw extracted CV text + a transient layout signal** (column count, scanned/image flag, verbatim date strings via `formatLayoutForPrompt`), never the master. Its job is to judge the real document an ATS/recruiter receives — column scramble, unreadable dates, buried credentials — and the cleaned master erases exactly those faults, so a master-fed teaser falsely passes both gates. **The layout signal is never stored:** it is computed at upload (`extractCvWithLayout`, geometry only the file shows), returned by `/api/upload-cv`, ridden in on the `analyse-background` request body, read once by the teaser, and discarded. A re-analysis with no fresh upload has no layout → the teaser judges on text alone (fine). The **deep pass** reads the master, where interpretive/structural calls belong on the strong model. The master build records facts only: it never decides overlapping roles are one consultancy — overlaps are surfaced as a `role_overlap` open question in `conflicts` for the user.
