...
- **Build:** the background analysis worker builds the master from the raw CV text the first time it's absent, persists it, cost-logs it, then feeds the master (not the raw CV) into both `analyzeTeaser` and `analyzeCvJob`. Falls back to raw text only if the build fails.
