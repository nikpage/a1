...
- **The analysis reads the MASTER.** One pass, one call (`analyzeCvJob`), handed `JSON.stringify(master)` and falling back to the raw `cv_data` text only if the master build failed. There used to be a landing-page **teaser** that read the raw CV for first impressions — page ORDER and SALIENCE, which the master's reordered, reconciled record deliberately dissolves — seeding a two-call deep pass. It was removed on 2026-08-16 along with the transient layout signal that fed its ATS gate: there are no landing-page visitors, the master is the source, and the CV is regenerated from it per job, so a first-impression read of an uploaded file described a document nobody will send.
...
- **Speaking & Lecturing and Publications are registered sections.** A record can hold dozens of talks and several articles, and none of it could reach a CV before 2026-08-21 because Layer 1 permits only registered headings and neither existed. For a candidate who ADVISED in a field rather than being employed in it, a conference talk on that subject is often the strongest evidence the record holds. Both render only when the blueprint's `section_order` asks for them, and the analysis is told to ask only where the subjects answer what the job is about.
...
- **`required_evidence` names the work THIS job makes essential** (`generation_framework.cv_blueprint`), each item paired with the requirement it answers, chosen by RELEVANCE and explicitly not by recency. A nested client engagement is named there in its own right: a CV generated for a Bitcoin company carried no crypto work at all, because the three crypto engagements live under an umbrella practice and the writer collapsed that entry into a handful of bullets, keeping the recent unrelated ones — while two- and three-month roles from 2011 held full dated entries for falling inside the recency window. Where several pieces of work answer the same core requirement they are ALL named; one example says the candidate touched that world once. Check 14 in `utils/cv-validate.js` warns when a named item never reached the document, matching each name by the stem its inflected forms share. **It is a warning, not a gate** — see PRODUCT.md "Known broken".
- **A capability the Summary names must have an INSTANCE in the record** (`CV_RULES.md` Layer 2, `prompts/cv-rules.js`, and the `summary_draft` instruction). The Summary is the only part of the CV whose claims attach to no role, so nothing contradicts a career-level sentence and every individual word is defensible: a CV claimed "experience designing and launching customer-facing digital products" over a record holding advisory work, a UX practice and QA strategy. The cause is the ad — the writer reaches for its framing where the record cannot answer it. Rule 6 of the verify pass is the net beneath this, not the home of the rule.
- **`ats_keywords_present` carries a PROOF QUOTE, checked against the master.** The analysis returns `[{ term, proof }]`; `splitProvenKeywords()` in `utils/cv-validate.js` verifies each proof against the record (folded substring, else every content word by 6-char stem, so CZ/PL inflection and Manager/Management never drop a true term), and `analyzeCvJob` applies it — at the single point every caller passes through, so the worker and `scripts/test-generate.mjs` behave identically. An unproven or quote-less term leaves the present list (barring it from `skills_to_highlight`) and joins `ats_keywords_missing`, which is the deny-list checks 24/26 read; with no job ad it is simply dropped, since that list must stay empty. This closes the leak that put "B2B", "Account Management" and "onboarding" on the KUBO CV: being wrongly "present" both fed the writer AND emptied the deny-list. Zero extra AI spend. Tests: `__tests__/keyword-provenance.test.js`.
...
- Wiring: `analysis.js` classifies the scenario FIRST — it is the decision that governs positioning and the whole blueprint — and emits `analysis.scenario_tags`, importing `scenarioList` + `scenarioHandling`. `cv-generator.js` no longer restates the per-scenario mitigations (see "What the CV IS" in `CV_RULES.md`); the tags reach the writer inside the blueprint and Layer 6 enforces the outcome. Gating by `hasJobText` keeps job-relative scenarios out of standalone passes.
...
- **ONE PASS: scenario, selection, framing.** The worker calls `analyzeCvJob(master, job, file)` once, for a VERIFIED session only, and saves the whole schema. It used to be three calls — a teaser plus a `blueprint` and a `review` delta merged by `mergeTeaserAndDelta`. The review half critiqued the uploaded document ("too long", "move this above the fold") which the app regenerates from the master anyway, so nobody could act on it; the teaser served landing-page visitors who no longer exist. Removed 2026-08-16, with `quick_wins`, `action_items` and `jobs_extracted`. **Log every analysis call** — the cost-logging rule has no exceptions.
...
What remains on screen: scores, ATS keywords present/missing, `red_flags`, `positioning_strategy` and `career_scenario`.
...
`utils/database.js` helpers; the one analysis pass; `--emphasise` / `--play-down`
...
**`scripts/golden-check.mjs` reads the finished DOCUMENT, which no test does.**
Every other test in this repo asserts a string is present in a PROMPT, so a CV
that was good three days ago has no artefact proving it was, and nothing fires
when it stops being — the owner finds out by generating one and reading it. The
harness runs the real pipeline over FROZEN inputs (`scripts/fixtures/golden/`: a
snapshot of the master and one ad, never the live DB, so two runs a week apart
see the same thing) and judges what the document must SAY, never its exact
words — a generated document never repeats verbatim, so exact diffing fails
every run and proves nothing. Each check in `expectations.json` is a defect
found by READING real output and carries the reason it exists; add one whenever
a new defect is found that way. `--check-only <dir>` re-reads saved output for
free. A full run costs one analysis plus one generation per document.
