...
export async function analyzeCvJob(cvText, jobText, fileName = 'unknown.pdf') {
  ...
  **IMPORTANT:**
  ...
  ${jobText?.trim() ? `### Step 5: Job Match Analysis

  **## Keyword Match**
  List job ad skills/keywords and how well the CV aligns.

  **## Inferred Keywords**
  Suggest 5–10 additional keywords the CV should contain to match intent (not just literal matches).

  **## Job Scenario**
  Compare CV to job. List: normal / pivot / overqualified — only if job ad is present.

  **## Positioning Strategy**
  Suggest any role/title changes or emphasis shifts to better match job ad.` : `// No job ad provided. Skip job match analysis.`}
