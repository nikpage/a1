...
- analysis.scan_snags: ARRAY of 2-3 ordered { "point": "", "detail": "" } steps that walk through HOW the recruiter's eye arrives at its snap doubt in the first seconds — each "point" is the thing the eye lands on (a real title, date range, or tenure from THIS CV, e.g. "Eye lands on \\"2016 – Present\\""), each "detail" is the one-line implication (e.g. "Reads as the current, primary role"). The last step should be the question that fires. ONLY populate when the scan raises a real doubt (typically scan_verdict "fail", or a genuine snag even on a pass); leave [] when the top-line is clean. Never invent facts; quote real top-line details.
