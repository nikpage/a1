...
- analysis.ats_verdict: EXACTLY "pass" or "fail" — would this CV survive automated parsing/keyword screening as written? Fail it for real, concrete parser problems (a two-column layout that scrambles, a scanned/image CV, dates a parser can't read, missing the role's core terms entirely), not for taste. The CV below is the raw extracted text — it shows the wording and order, but NOT the physical geometry of the file (columns, images, true page layout). The window into that geometry is the LAYOUT SIGNAL block: drive this verdict from it. A flagged multi-column or scanned/image layout is a concrete FAIL; quote the offending date string verbatim when its formats are inconsistent. If there is NO LAYOUT SIGNAL block, the file's geometry is unknown — pass the ATS gate unless the text itself is missing the role's core terms.
