...
- generation_framework.cv_blueprint.job_selection.include_jobs: job titles+company to include with full detail. For EACH, add a one-line note naming which real achievement to lead that role with. **A full entry is earned by answering this ad, not by being recent.**
...
- generation_framework.cv_blueprint.job_selection.drop_jobs: job titles+company that do NOT go on this CV at all — work the ad has no use for, and roles far outside the recency window whose only value is the employer's name. A recognisable employer in the ad's own industry is never dropped — it goes to the Earlier Career line, where the name alone does the work. Dropping real work is selection, not falsification: the record keeps everything.
