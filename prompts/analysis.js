...
- generation_framework.cv_blueprint.job_selection.earlier_career: ARRAY, ORDERED, AT MOST SIX, of the roles outside the recency window that still earn a line on this CV — each item the string "Title, Employer" exactly as the record spells both. This is the Earlier Career roster and the writer prints it VERBATIM, so the choice is yours and it is made here, once. **Order and choose by the strength of the employer's NAME and the relevance of the title to this ad, never by recency.** For a finance or crypto ad, a candidate who worked at Morgan Stanley and Wells Fargo has two names a screener recognises on sight, and spending the roster on the three most recent old roles instead is the most expensive cut the document can make: those names cost one line each and carry no date, so they cost no age signal either. A role outside the window that is not on this list does not appear on the CV at all. Empty array only where the record holds no role outside the window. Dropping real work is selection, not falsification: the record keeps everything.
