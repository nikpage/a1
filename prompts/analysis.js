...
${hasProfile ? `- analysis.keywords_for_this_job: ARRAY of { "term": the keyword, "proof": the exact phrase from the record that proves it }, most relevant to THIS ad first. Copy the settled PROVEN terms above with "proof" empty (already checked), then add what this ad needs that the record genuinely proves — including the employer's own domain — each with a VERBATIM proof quote. Speaking and publications prove a domain like any role. The proof is checked in code; an unquotable term is dropped.
...
- generation_framework.cv_blueprint.section_order: ordered list of section names the CV writer must follow. Use ONLY these standard, ATS-parseable names — Summary, Core Competencies, Skills, Work Experience, Consulting Engagements, Projects, Speaking & Lecturing, Publications, Education, Certifications — and no others; a creative section name breaks parsing. Include "Core Competencies" only for a Career Pivot, and "Projects" only for a Career Pivot or an Under-qualified candidate with evidenced projects. Include "Speaking & Lecturing" and "Publications" only when the master records them and their subjects answer this job. Include "Consulting Engagements" whenever the master holds work_experience[].fractional_engagements[] that answer this ad — a client engagement is not a role and cannot be shown as one, so without this section it survives only as one bullet inside its umbrella practice, which is how it disappears. Place it directly after Work Experience.
- generation_framework.cv_blueprint.top_three_achievements: ARRAY of exactly the THREE strongest achievements this candidate has FOR THIS JOB, drawn verbatim in substance from the master's experience[].achievements[] and each naming the role and employer it came from. These go in the CV's impact zone (the first ~120 words), so pick for force of evidence against this ad — a real number or a concrete shipped deliverable beats a duty. **Relevance to this ad outranks having a number.** Advisory and consulting work often carries no revenue or percentage figure, and picking on the presence of a digit hands the impact zone to whatever work happens to be measurable rather than to the work this employer cares about. Where such an achievement is the strongest answer to the ad, quantify its SCOPE from the record instead — team size, how long it ran, the client's size or industry. Never write one the record does not prove.
- generation_framework.cv_blueprint.job_selection.include_jobs: job titles+company to include with full detail. For EACH, add a one-line note naming which real achievement to lead that role with. **A full entry is earned by answering this ad, not by being recent.** A role the ad has no use for goes to condense_jobs or to the earlier_career roster, and the room it frees goes to work that answers the ad, including older work.
...
- generation_framework.cv_blueprint.job_selection.earlier_career: ARRAY, ORDERED, AT MOST SIX, of the roles outside the recency window that still earn a line on this CV — each item the string "Title, Employer" exactly as the record spells both. This is the Earlier Career roster and the writer prints it VERBATIM, so the choice is yours and it is made here, once. **Order and choose by the strength of the employer's NAME and the relevance of the title to this ad, never by recency** — those names cost one line each and carry no date. A role outside the window that is not on this list does not appear on the CV at all. Empty array only where the record holds no role outside the window. Dropping real work is selection, not falsification: the record keeps everything.
- generation_framework.cv_blueprint.required_evidence: ARRAY, ordered most important first, of the evidence THIS JOB makes essential — the work whose absence would make the CV fail to answer this ad. Each item is { "evidence": the employer or engagement name exactly as the record spells it, "requirement": the ad requirement or theme it answers }. Nested client engagements (work_experience[].fractional_engagements[]) are named here in their own right — they are what the Consulting Engagements section prints, so an engagement this ad needs must be named here or it does not reach the CV at all. **Choose by relevance to this ad, never by recency**, and where several pieces of work answer the same core requirement, name them all. 3-6 items. Every item must be real work in the record.
- generation_framework.cv_blueprint.evidence_from_speaking: ${hasJobText ? `ARRAY of the speaking, lecturing, workshop and publication entries from the record that answer THIS ad's SUBJECT — each written exactly as the record spells the event and topic. **Chosen by subject, never by recency or prestige.** Up to six. Empty array where the record's subjects have nothing to do with this job — then the section is omitted from section_order too.` : `MUST be an empty array. With no job ad there is no subject to select against.`}
- generation_framework.cv_blueprint.headline_draft: the target-role HEADLINE that sits under the candidate's name — max ~8 words, tone-neutral, no full sentence, no punctuation at the end. It names WHAT THIS PERSON IS (role/discipline, optionally one domain or specialism the record proves), e.g. "Senior Backend Engineer | Payments Platforms".${hasJobText ? ' Make it job-aware: point it at the advertised role, but ONLY using a label the record genuinely earns — never borrow the ad\'s title if the record does not support it.' : ' With no job ad, name the role the record itself proves.'} HARD RULES: never a duration ("12+ years", "a decade of"), never a title, seniority, domain or specialism the record does not evidence, never a slogan or adjective soup ("results-driven professional"). If the record does not support a distinct headline, use the candidate's own most-recent real job title.${hasJobText ? ' **The specialism it names is the one THIS ad is about**, wherever the record proves it.' : ''}
- generation_framework.cv_blueprint.summary_draft: WRITE A STRONG, IMPACT-FIRST PROFESSIONAL SUMMARY DRAFT — max 3 sentences, tone-neutral, no "Seeking to" / "Looking to" openers, no repeated phrases. Lead with the candidate's strongest proof for this job (scope, scale, results). **The first clause names what this person DOES and their hardest evidence for it — never a qualifying fact.** Languages, nationality, location, work authorisation and years-of-service are filters a screener checks, not what persuades: keep them out of the first clause. Plain, specific language — strong action verbs are fine, but cut empty filler ("results-driven", "proven track record", "passionate about", "dynamic", "synergy"). The CV writer adapts this into the requested tone, so make it factual and dense, not stylised.${hasJobText ? ' **Every capability it names must have an INSTANCE in the record — a role, a client, a piece of work where the candidate did that thing.** Where the record cannot answer a requirement, the summary is silent on it. It answers what THIS ad asks for, in the order the ad cares about it, using the evidence named in required_evidence.' : ''}
- generation_framework.cv_blueprint.skills_to_highlight: 8-12 specific skills drawn ONLY from transferable_skills and ats_keywords_present (skills the candidate genuinely has), ordered by relevance to this ad. **The employer's own core domain leads the list wherever the record proves it**, and each skill is named in the AD'S OWN WORDS where the record genuinely earns it — a relabel of proven work, never a claim. NEVER pull from ats_keywords_missing — a skill the record cannot prove must never appear here.
