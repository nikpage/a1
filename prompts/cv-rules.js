...
- **A capability the Summary names must have an INSTANCE in the record.** The value proposition says what this person DOES, and the trap it sets is that no single line of the record contradicts a career-level sentence — so a claim the record never evidences reads as reasonable and passes every check that compares one claim to one role. Before writing a clause naming what they do, point to the entry that proves it: the role, the client, the piece of work where it happened. If nothing in the master shows it, it does not go in, however precisely it answers the ad. Advising is advising, building a practice is building a practice, running research is running research; none of them becomes "designing and launching products" because the ad asked for that. This is the invariants applied to the Summary, where they are hardest to see: it is the one part of the CV whose claims attach to no role.
...
export function coverMatchingRule(hasJobText) {
  if (!hasJobText) return '';
  return `- **Answer what the ad asks, with real evidence — DEPTH, NOT COVERAGE (Layer 3).** The letter's argument rests on the ad's most important requirements answered by this candidate's strongest evidenced work. Go DEEP on TWO of them: what the situation was, what they did, what changed. A letter that touches five employers is a career summary, and the CV already lists everything — repeating it in prose is the commonest way a cover letter wastes itself. A third piece of evidence only where it genuinely earns its place; a fourth means the argument has been lost. Everything you leave out is still on the CV. PAIR each one in the prose — the requirement and the evidence in the same breath — never list them, never give each its own paragraph. Make no claim the CV does not also support, and never answer a requirement the master does not evidence: an unevidenced requirement is a gap reported to the candidate, not a sentence in the letter.
- **Name the claim in the first paragraph (Layer 3).** One sentence saying why THIS candidate and THIS role belong together. Everything after it proves that sentence. If the reader has to infer the claim from four paragraphs of history, the letter never made one.`;
}
...
export function coverOpeningRules(contact = '') {
  ...
  return `- **The salutation is a fact, not a formula (Layer 3).** ${named
    ? `The ad names its contact: address the letter to ${named}. Using the name the ad printed is the first evidence this letter was written for THIS application; "Dear Hiring Manager" over a named contact is not neutral politeness, it is a tell.
  **Decline the name into the salutation's own language.** English takes the name as it stands ("Dear ${named}"). CZECH AND POLISH DO NOT: a Czech salutation is in the VOCATIVE (5. pád) and a nominative name in that slot is a visible grammatical error to every reader — "Vážený pane Nováku," never "Vážený pane Novák,"; "Vážená paní Nováková," ; "Vážený pane Ambroži," for Ambrož, "Vážený pane Sládku," for Sládek, "Vážená paní Krejčí," (indeclinable, unchanged). Polish takes the vocative too: "Szanowny Panie Kowalski," / "Szanowna Pani Kowalska,". Where the ad gave a first name only, the same rule applies to it ("Vážený pane Petře," / "Vážená paní Jano,"). Where the name's gender or declension is genuinely unclear, use the neutral form for that language rather than guessing a wrong ending.`
    : `The ad names nobody, so use the neutral form in the letter's own language — "Dear Hiring Manager", "Vážená paní, vážený pane," in Czech, "Szanowni Państwo," in Polish. NEVER guess, infer or invent a name.`} No titles like Mr./Ms. (Czech "pane"/"paní" and Polish "Panie"/"Pani" are part of the salutation, not titles, and are required.)
- **Open on something real (Layer 3).** The opening rests on a fact the record holds — something this candidate did, built or ran that this role needs. BANNED as an opening: any description of the act of applying ("I am writing to apply", "I am excited to submit"); **any GENERAL TRUTH about the role, the product or the industry** ("Building recommendation engines requires aligning backend mechanics with human behaviour", "Great products are built where data meets empathy") — it is a thesis statement about the job rather than a fact about the applicant, it could open fifty other candidates' letters for this ad, and the reader learns nothing from it; any identity claim standing in for evidence ("As a seasoned leader", "As an accomplished professional") — the epithet ban binds the letter exactly as it binds the CV; and **any invented scene-setting** — a remembered moment, a mood, a realisation, a year, a place or any colour the master does not record. That last one is the real defect behind the anecdote: not that a letter tells a story, but that a story needs detail and a writer without detail invents it. "I remember one of my first projects…", "Vzpomínám si na…", "It was 2019 when…" are barred wherever the record holds no such project and no such year.
- **A build is allowed where the candidate's own voice builds (Layer 3 + Layer 2).** If their recorded voice reaches its point rather than leading with it, the letter may open that way — provided every sentence it builds on is a fact from the record, and provided the ad's own register carries it. This is not a licence for throat-clearing: a build made of real facts is the candidate's manner, a build made of atmosphere is invention. Where there is no recorded voice, lead with the thing done, where it was done, and what came of it.
