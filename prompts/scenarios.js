...
const BASE_SCENARIOS = {
  ...
  'Senior Portfolio / Independent Consultant': {
    detect: 'A senior candidate whose recent record mixes a standing independent practice (consulting/advisory/freelance) with one or more shorter corporate engagements, often concurrent/overlapping. The short stints are interventions, not instability.',
    handling: 'This is the case a naive scan misreads as job-hopping. Do NOT treat concurrency or a standing consultancy as a red flag — the consultancy is the CONTINUOUS SPINE that closes apparent gaps between corporate roles. Position the candidate as an established operator who is engaged for focused, high-value work. Foreground the deep anchor roles (long tenures, marquee employers/clients) so the verdict rests on the real depth, not the most recent short engagement.',
    generation: 'Make the standing consultancy the continuity thread: show it as an ongoing practice ("[start] - Present") that spans the corporate engagements rather than burying it mid-list. Frame short corporate stints as defined engagements/mandates, not failed permanent roles. Lead the experience section with the strongest anchor (longest tenure or biggest name), not strict reverse-chronology if that would open on a sub-year stint. Surface marquee clients/employers high. Never invent scope or duration.',
  },
};
