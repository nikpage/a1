...
const BASE_SCENARIOS = {
  ...
  'Older Applicant': {
    ...
    cover: 'State NO cumulative career total — no \'25+ years\', \'over two decades\', \'a decade of\'. Duration scoped to one role or engagement is fine. Draw evidence from the recent, relevant work the CV shows; do not narrate the early career the CV collapsed into the Earlier Career section.',
    generation: 'Apply the 10-15 year window strictly: full detail only for roles inside it. Collapse ALL older roles into an undated "Earlier Career" section, ONE BULLET PER ROLE, reverse-chronological: each bullet is "Title, Employer" plus " — Location" where the location itself carries weight (a major market the candidate genuinely worked in). NAME the employers individually as the master records them: a category ("financial institutions and tech companies") is NOT a substitute, and dissolving a marquee employer into one throws away the strongest thing in that section. The bullets carry no dates, so a real market costs no age signal — but print a location ONLY where the master records one for that role, NEVER inferred from the employer\'s name however well known its home city. No dates, no achievements, no prose: a bullet that describes what the role involved is a Work Experience entry smuggled past the window. AT MOST SIX bullets, chosen for the strength of the name and the relevance of the title rather than simply the six most recent; older roles beyond that are dropped silently, with no "and others" line and no count. Format the section as its own experience block using the SAME markdown structure as every other role (#### **Earlier Career** heading, then the bullets) so it renders with identical styling — never as a plain paragraph; it is the only undated entry permitted on the CV. Strip graduation years from EVERY Education entry — all of them or none, never selectively. Never state a CUMULATIVE career total anywhere — "25+ years\' experience", "over two decades in banking", "a decade of expertise": that is one number the screening sort can act on before it reads anything. Duration scoped to a SINGLE role, project or engagement stays — "five years running the Prague platform team", "twelve years at Deutsche Bank" — because depth lives in scope, marquee names and long tenures inside the visible window, and cutting those would strip out the very evidence this override protects. Keep every date that IS shown exactly as the master records it — compress by selection, never by altering a real date.',
  },
