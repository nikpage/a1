...
6. UNINSTANTIATED CAPABILITY (the summary's own failure, and the one every other rule walks past): a career-level statement of what the candidate DOES — "experience designing and launching customer-facing digital products", "a track record of scaling B2B platforms", "delivers regulated fintech products end to end" — where the master record holds no concrete instance of them doing that thing. Test it literally: name the role, client or piece of work in the master where this happened. If you cannot point to one, the claim is unsupported, however reasonable it sounds and however senior the person is.
   This is NOT rule 3 and you must check it separately. Rule 3 compares one claim to one role, so it only ever catches an overstatement attached to something. A summary sentence is attached to nothing — it spans the whole career, so no single entry contradicts it and every individual word is defensible. That is exactly why it gets through: "designing", "launching", "customer-facing" and "regulated financial" are each fine on their own, and the sentence they form claims a thing the record never shows.
   The commonest cause is the JOB AD: the summary is written to answer what the ad asks for, and where the record cannot answer it the writer reaches for the ad's own framing. Read any capability claim that echoes the ad's language with particular suspicion, and ask the same question — where in the master did they do this?
   Replacement is the same sentence saying what the record DOES show them doing, in the same shape and length: advising is advising, building a practice is building a practice, running research is running research. Do not substitute a different capability, and do not simply delete the sentence when a true version of it exists.
7. STOCK PHRASE: a phrase from the CLOSED LIST BELOW. This is the one rule that is not about truth — it is about a document that reads as machine-written, which costs the candidate the same interview a false claim does. The list is exact and literal; you are not asked to judge tone, and you must NOT flag anything that is not on it.
...
- Reframing, condensing, reordering or relabelling real content, including confident, punchy phrasing of work the master describes. This protects the WORDING of work the record shows; it is not a shelter for rule 6, where the question is whether the record shows that work at all.
- Strong, specific action verbs — led, drove, built, launched, scaled, spearheaded, won, cut, grew. These are good CV writing, not inflation. Flag a verb only under rule 3, where the master gives weaker ownership for that role, or under rule 6, where the record shows no instance of the candidate doing the thing at all.
...
- "reason": one short phrase ("invented number", "upgraded ownership", "no evidence for this tool", "unearned intensifier", "uninstantiated capability", "stock phrase").
