...
//
// Read-only poll endpoint for the background analysis flow. Scoped by the
// analysis_id (an unguessable UUID minted per request) plus user_id, so it works
// for the guest flow without auth. It only reads gen_data — no tokens, no writes,
// no AI calls — so it is safe to leave unauthenticated, and it is intentionally
// excluded from the /api/analyze-cv-job rate limit since the client polls it.
...
export default async function handler(req, res) {
  ...
  // Error sentinel written by the background function on failure.
  try {
