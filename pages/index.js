...
export default function HomePage() {
  ...
  return (
    <div className="container">
      ...
      {/* Top meta in two-column grid */}
      <div style={{ margin: '2rem 0 2rem 0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', rowGap: '0.75rem', alignItems: 'start' }}>
          <div style={{ fontWeight: 'bold' }}>Name:</div>
          <div>{cvMetadata.full_name || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Phone:</div>
          <div>{cvMetadata.phone || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Email:</div>
          <div>{cvMetadata.email || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Languages:</div>
          <div>{cvMetadata.languages?.join(', ') || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Years Experience:</div>
          <div>{cvMetadata.years_experience || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Seniority:</div>
          <div>{cvMetadata.seniority || "None Found"}</div>

          <div style={{ fontWeight: 'bold' }}>Current Role:</div>
          <div>{cvMetadata.current_role || "None Found"}</div>
