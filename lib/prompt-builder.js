...
export function buildCVFeedbackPrompt(documentType, targetIndustry = 'general', country = 'cz') {
    ...
    const europassGuidance = {
        structure: 'Use Europass format with sections: Personal Information, Work Experience, Education, Skills, Additional Information',
        languages: 'Include detailed language proficiency levels (A1-C2)',
        skills: 'Separate digital, organizational, and communication skills sections'
    };
    ...
    const internationalPointers = {
        europass: europassGuidance,
        general: `For ${country}: ${countryNorms[country] || `Use regionally appropriate formatting for ${country || 'the candidate\'s location'}.`}. Consider local employment laws regarding personal information disclosure.`
    };
    ...
    promptBase += `Start with an honest, rigorous rating (1-5 stars - reserve 5 stars for truly exceptional CVs) followed by:\n
...
🌍 [International Formatting] - ${internationalPointers.general}\n`;
...
Also generate (but don't show to user) a JSON object with:
{
  "ats_score": number,
  "overall_rating": number,
  "action_points": ["specific action 1", "specific action 2", ...],
  "country_specific_notes": ["note 1", "note 2", ...],
