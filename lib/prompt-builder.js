...
function detectCareerScenarios(jobDetails, metadata, cvText) {
  const scenarios = [];
  if (metadata?.recent_graduate) scenarios.push('recent_grad');
  if (metadata?.career_pivot) scenarios.push('pivot');
  if (metadata?.returning_to_work) scenarios.push('career_returner');
  if (metadata?.older_applicant) scenarios.push('older_applicant');
  if (metadata?.overqualified) scenarios.push('overqualified');
  return scenarios;
}
...
// Add this after the CRITICAL GUIDELINES section
switch (tone) {
  case 'formal':
  ...
  // 🟩 ADDING feedbackData if present
  if (feedbackData) {
    promptBase += `
...
- Frame extensive experience as bringing valuable perspective to the role`;
  ...
  promptBase += `\nFORMAT: Create a professional CV with clear sections for Summary, Skills, Experience, Education, and any other relevant categories. Use bullet points for achievements and experience.`;
...
Your job is to analyze the provided CV and job description below, and produce a comprehensive list of keywords for ATS optimization and career pivot scenarios.

**Your output must be a valid, minified JSON object matching this exact schema:**
{
  "cvKeywords": ["keyword1", "keyword2", ... up to 30],
  "jobKeywords": ["keyword1", "keyword2", ... up to 30],
  "suggestedKeywords": [
    { "keyword": "example1", "confidence": "high" },
    { "keyword": "example2", "confidence": "medium" },
    ...
  ]
}

**Guidelines:**
1️⃣ Extract job keywords directly from the job description text — capture explicit keywords and inferred ones.
2️⃣ Merge these extracted job keywords with the provided job keywords list (if any) to create the final jobKeywords list.
3️⃣ Use the provided cvKeywords (if any) as-is in the cvKeywords list.
4️⃣ Treat any items in the primary_skills list as high-priority or most important when generating suggestedKeywords.
5️⃣ Generate the suggestedKeywords list by analyzing:
   - Matches and synonyms between primary_skills and jobKeywords (high confidence)
   - Related or similar concepts bridging the CV and the job (medium confidence)
   - Best guesses or inferred connections not explicit in the job or CV (low confidence)
6️⃣ Classify each suggested keyword by confidence ("high", "medium", "low").
7️⃣ Output only the JSON object as specified — no extra text, no preamble, no code block formatting.
...
**Here are the Job Keywords (from DB):**
---
${jobKeywords.join(', ')}
---
