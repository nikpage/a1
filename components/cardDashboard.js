// /components/cardDashboard.js

import { useState } from 'react';

export default function CardDashboard({ userId, onExtract }) {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      if (onExtract) onExtract(data, initialToggles);
    } catch (err) {
  ...
  const handleToggle = (field) => {
    ...
    setToggles(updated);
    if (onExtract) onExtract(metadata, updated);
  };

  const handleFieldChange = (field, value) => {
    const updatedMeta = { ...metadata, [field]: value };
    setMetadata(updatedMeta);
    if (onExtract) onExtract(updatedMeta, toggles);
  };
