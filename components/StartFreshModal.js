...
import LoadingModal from './LoadingModal' // Import the new modal
...
export default function StartFreshModal({ user_id, onClose, onSubmit, onStartFresh }) {
  ...
  const [showLoadingModal, setShowLoadingModal] = useState(false) // New state for modal
  const [loadingModalMessage, setLoadingModalMessage] = useState('') // New state for modal message
  const [loadingModalTitle, setLoadingModalTitle] = useState('') // New state for modal title
  ...
  const handleCvUploadFromModal = async (fileToUpload) => {
    setShowLoadingModal(true) // Show loading modal
    setLoadingModalTitle('Uploading New CV');
    setLoadingModalMessage('Processing your new CV for analysis...');
    ...
    try {
      ...
      // You might want to display an error within StartFreshModal or pass it up
    } finally {
      setShowLoadingModal(false); // Hide the loading modal
    }
  };
  ...
  return (
    <>
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-gray-400 transition-colors">
                      <div className="text-center">
                        <CVUploader
                          user_id={user_id}
                          onUpload={(newFile) => handleCvUploadFromModal(newFile)} // Pass the file to a new handler
                          compact
                          selectedCv={cvText}
                        />
      ...
      {showLoadingModal && ( // Render the LoadingModal
        <LoadingModal
