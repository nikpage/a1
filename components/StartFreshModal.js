          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4 mb-6">
              <CVUploader
                user_id={user_id}
                onUpload={() => {
    onStartFresh(); // clear all tabs and doc versions
    document.querySelector('[data-tab-content]')?.replaceChildren(); // wipe tab DOM
    fetchCVs(); // reload CVs
    onClose(); // close modal
    setTimeout(() => {
      const latest = [...document.querySelectorAll('[data-cv-id]')].pop();
      if (latest) latest.click();
    }, 300); // select new analysis

    const event = new CustomEvent('clear-analysis');
    window.dispatchEvent(event); // trigger external listener
  }}
