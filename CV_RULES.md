...
**Earlier Career form.** One bullet per role, reverse-chronological, each bullet `Title, Employer` plus ` — Location` where the location earns its place. No dates, no achievements, no prose: a bullet that describes what the role involved is a Work Experience entry smuggled past the recency window. **At most six bullets**, chosen for the strength of the name and the relevance of the title, not simply the six most recent — the section exists so a recognisable employer is not lost, and a longer list drags the reader backwards through a career the window is meant to close. Where more roles fall outside the window than the section prints, the remainder are simply omitted; no "and others" line, no count. These bullets are outside the Layer 2 bullet ceilings and the metric-fallback share, which govern Work Experience only. A location prints only where the master records it from the source — it is never inferred from the employer's name, however well known its home city. **Which roles appear is decided by the analysis, not by the writer.** The blueprint's `job_selection.earlier_career` is an ordered roster of at most six `Title, Employer` entries, chosen against this ad, and the generated CV prints exactly that list in exactly that order — the section is the one place where a recognisable employer survives the recency window, and leaving the pick to the writer at write time produced a CV that printed the three most recent unknown employers and lost Morgan Stanley and Wells Fargo on a finance application. Layer 6 fails the document when a rostered employer is missing.

**Client engagements have their own section.** Where the master nests client work inside an umbrella practice (`work_experience[].fractional_engagements[]`), that work does NOT compete for bullets inside the parent entry. A Work Experience entry is title / employer / dates / bullets, and a nested engagement is not a role, so the only place a client could appear was as one bullet among a handful — which is why three crypto engagements vanished from a CV generated for a crypto employer. The document instead carries a **Consulting Engagements** section, placed after Work Experience, one entry per client: client name, dates, one line saying what the engagement was, and one or two bullets. It renders only where the blueprint's `section_order` asks for it, and its contents are the engagements the blueprint named in `required_evidence` — chosen against this ad, not by the writer. The umbrella entry keeps its own dates and bullets; the engagements are shown, not moved. This is standard practice for a consultant with several clients, and it is the only shape in which relevance can compete at all: a client that gets its own entry is visible, and a client reduced to a bullet is not.

**Engagement bullets are quantified by SCOPE where no outcome number exists.** Advisory work often carries no revenue or percentage figure, and under a metric-fallback rule that recognises only outcome numbers it loses the impact zone automatically to any achievement that has one — relevance never gets to compete. Team size, engagement duration, the client's size and its industry are real, checkable numbers from the record and count as quantification. They are never invented and never upgraded into an outcome: "advised a 12-person product team over nine months" is quantified; "grew their revenue" is fabrication unless the record says so.
