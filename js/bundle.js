  ...
  var DocumentUpload = class {
    ...
    setup() {
      ...
      ["dragleave", "dragend"].forEach(
        (evt) => this.dropZone.addEventListener(evt, () => this.dropZone.classList.remove("drag-over"))
      );
    ...
    async runAnalysis() {
      ...
      try {
        ...
        const res = await fetch("/api/analyze", {
          ...
          body: JSON.stringify({ text, userId: userId2 })
        });
    ...
    showFeedback(feedback) {
      ...
      if (!lang)
        feedback.languages = "[MISSING: CV LANGUAGE]";
      if (isNaN(years) || years < 0)
        feedback.years_experience = "[INVALID: YEARS EXPERIENCE]";
      const html = `
      ...
      submitButton.onclick = async () => {
        ...
        try {
          ...
          const res = await fetch("/api/second-stage", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId: userId2, metadata, cv_body: cvText })
          });
    ...
    formatAIText(text) {
      const lines = text.replace(/---/g, "").replace(/^#+\s*/gm, "").replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/\*(.+?)\*/g, "<em>$1</em>").split("\n");
      let html = "";
      let inList = false;
      let inSubList = false;
